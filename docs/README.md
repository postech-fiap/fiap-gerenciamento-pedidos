...
```bash
# Criar Secrets
kubectl create secret generic db --from-literal=username=<user-name> --from-literal=password=<user-password>
kubectl create secret generic mp --from-literal=user-id=<user-id> --from-literal=external-id=<external-id> --from-literal=token=<token>
